...
(function () {
    ...
    angular.module('app').factory('LogFactory', ['$http', function ($http) {

        var _pesquisaLog = function _pesquisaLog(param) {
...
(function () {
    ...
    angular.module('app').controller('listarLogsCtrl', ['$scope', '$http', 'LogFactory', '$cookies', '$filter', '$location', function ($scope, $http, LogFactory, $cookies, $filter, $location) {
        ...
        var init = function init() {
            ...
            if (verificaCookies() == false) {
                ...
                $scope.filtro = $cookies.get('log_acessos_usuario') == "" && $cookies.get('log_acessos_ip') == "" && $cookies.get('log_acessos_tipo') == "" && $cookies.get('log_acessos_perfil') == "" ? false : true;
        ...
        var verificaCookies = function verificaCookies() {
            ...
            if (typeof $cookies.get('log_acessos_usuario') == "undefined" && typeof $cookies.get('log_acessos_ip') == "undefined" && typeof $cookies.get('log_acessos_tipo') == "undefined" && typeof $cookies.get('log_acessos_perfil') == "undefined") {
                return false;
            }
            return true;
        };
        ...
        $scope.pesquisaLog = function (pesquisa) {
            ...
            LogFactory.pesquisaLog(pesquisa).then(function mySuccess(response) {

                $scope.lista = response.data.logs.data;

                // Calcula alguns parametros da paginação.                
                var de = response.data.logs.current_page == 1 ? 1 : response.data.logs.total < pesquisa.limite ? (response.data.logs.current_page - 1) * pesquisa.limite + response.data.logs.total : (response.data.logs.current_page - 1) * pesquisa.limite + 1;

                var ate = pesquisa.limite == 0 ? response.data.logs.total : response.data.logs.total < pesquisa.limite ? response.data.logs.total : response.data.logs.current_page * pesquisa.limite;

                var totalDePaginas = response.data.logs.total % pesquisa.limite === 0 ? response.data.logs.total / pesquisa.limite : Math.floor(response.data.logs.total / pesquisa.limite) + 1;
