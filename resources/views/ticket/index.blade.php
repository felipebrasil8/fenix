...
@section('titulo', 'Tratar tickets')

@section('content')

<migalha titulo="Tratar tickets" descricao="Pesquisar ticket"></migalha>
    
<section class="content" ng-controller="listarTicketCtrl"> 
                ...
                <form role="form" ng-submit="pesquisaTicket(filter)">                
                    ...
                    <div class="box-body row">
                        ...
                        <div class="form-group col-md-3">
                            ...
                            <select class="form-control input-sm" ng-model="filter.usuario_solicitante" ng-options="x.nome for x in usuarios" ng-init="verificaCookiesSolicitante();">
                            </select>                            
                        </div>
                        @endif
                        <div class="form-group col-md-3">
                ...
                <div class="table-responsive">                    
                    ...
                    <table class="table table-striped table-hover">
                        <tr id='cabecalho-da-pagina'>
                            ...
                            <th ng-click="defineFiltro('usuario_solicitante_nome')" class='sortable'>
                                Solicitante
                                <span class='fa fa-caret-up' ng-class="{'selecionado': (filter.ordem == 'asc' && filter.coluna == 'usuario_solicitante_nome') }"></span>
                                <span class='fa fa-caret-down' ng-class="{'selecionado': (filter.ordem == 'desc' && filter.coluna == 'usuario_solicitante_nome') }"></span>
                            </th>
                        ...
                        <tr ng-repeat="ticket in lista">
                            ...
                            <td><a href="/ticket/@{{ ticket.id }}" title="Visualizar" style="color:#333;">#@{{ ticket.codigo }}</a></td>
                            
                            <td>@{{ ticket.data }}</td>

                            <td ng-class="{'text-bold': ticket.usuario_solicitante_id == usuario_logado}">@{{ ticket.usuario_solicitante_nome }}</td>

                            <td>@{{ ticket.assunto }}</td>

                            <td>@{{ ticket.categoria_nome_pai }}</td>

                            <td ng-class="{'text-bold': ticket.usuario_responsavel_id == usuario_logado}">@{{ ticket.funcionario_responsavel_nome }}</td>
                            ...
                            <td class="text-center">
                                @if( $visualiza )
                                <a href="/ticket/@{{ ticket.id }}" title="Visualizar"><i class="fa fa-file-text-o"></i></a>
                                @endif
                            </td>
   ...
   <!-- Formulario com trigger em angularjs -->
    <form name="exportar" method="post">
        ...
        <input type="hidden" value="@{{filter.codigo}}" name="codigo">
        <input type="hidden" value="@{{filter.aberto}}" name="aberto">
        <input type="hidden" value="@{{filter.assunto}}" name="assunto">
        <input type="hidden" value="@{{filter.departamento.id}}" name="departamento">
        <input type="hidden" value="@{{filter.usuario_responsavel.id}}" name="usuario_responsavel">
        <input type="hidden" value="@{{filter.usuario_solicitante.id}}" name="usuario_solicitante">
        <input type="hidden" value="@{{filter.categoria.id}}" name="categoria">
        <input type="hidden" value="@{{filter.statuse.id}}" name="statuse">
        <input type="hidden" value="@{{filter.prioridade.id}}" name="prioridade">
    </form>
