<template> 
    ...
    <div class="box box-default">
        <div class="table-responsive">  
            ...
            <table class="table table-hover" style="table-layout:fixed;min-width: 1000px; margin-bottom: 0">
                <thead>
                    <tr>
                        <th 
                            class="pointer" 
                            width="40%" 
                        > 
                            <i class="fa fa-square-o" style="font-size:12px; color: rgb(43, 140, 184);" aria-hidden="true" v-if="abreTodos == 0" @click="abreTodosItens()" data-toggle="tooltip" data-placement="right" data-original-title="CLIQUE PARA OCUTAR OS ITENS"></i>
                            <i class="fa fa-plus-square" style="font-size:12px; color: rgb(43, 140, 184);" aria-hidden="true" v-else-if="abreTodos  == 1" @click="abreTodosItens()" data-toggle="tooltip" data-placement="right" data-original-title="CLIQUE PARA EXIBIR OS ITENS"></i>
                            <i class="fa fa-minus-square" style="font-size:12px; color: rgb(43, 140, 184);" aria-hidden="true" v-else-if="abreTodos == 2" @click="abreTodosItens()" data-toggle="tooltip" data-placement="right" data-original-title="CLIQUE PARA VOLTAR A EXIBIÇÃO PADRÃO"></i>
                            <span @click="ordenacao($event, 1 )"  class="naoSelecionavel">
                                     <!-- <i class="fa " 
                                    style="font-size:12px;" 
                                    :style="item.itens.length == 0 ? 'color:#ccc':'color:#2b8cb8'" 
                                    :class="aberto[item.id]?'fa-minus-square':'fa-plus-square'" 
                                    aria-hidden="true" 
                                    @click="item.itens.length == 0 ? '':abreItens(item.id)"
                                    ></i> -->
                                ...
                                <span style="vertical-align: super; display: inline-grid;">
                                    <i class="fa fa-caret-up " style="height:7px;" :class="ordenacaoIcone( 1 ) == 'asc'?'sortSelectd':'sortDisablede' "></i>
                                    <i class="fa fa-caret-down " style="height:7px;" :class="ordenacaoIcone( 1 ) == 'desc'?'sortSelectd':'sortDisablede'" ></i>
                                </span>   
...
<!--                            <span v-if="ordenacaoIcone( item.coluna ) == false" class='fa fa-sort'></span>
                ...
                <tbody v-for="item in dados.data">
...
<script>
    ...
    export default{
        ...
        props:['dados', 'url', 'filtro', 'canObj', 'cookiePrefixo', 'thead', 'status', 'acoes', 'abreTodos'],
        ...
        methods: {
            ...
            abreTodosItens( )
            {   
                let abre = this.abreTodos > 1 ? 0 : this.abreTodos+1
                this.$emit('abreTodosItens', abre )
            },
