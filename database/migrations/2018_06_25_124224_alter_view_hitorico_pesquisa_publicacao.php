...
class AlterViewHitoricoPesquisaPublicacao extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        ...
        DB::statement("CREATE OR REPLACE VIEW historico_pesquisa_publicacao_view AS
            ...
            SELECT 
                ...
                (CASE WHEN pbh.qtde_resultados > 0 THEN '-'
                        WHEN pbh.tratada = false THEN 'NÃO'
                            ELSE 'SIM'
                                END) as tratada,
                (CASE WHEN pbh.qtde_resultados > 0 THEN null
                        WHEN pbh.tratada = false THEN null
                            ELSE pbh.updated_at::date
                                END) as data_tratada,
                (CASE WHEN pbh.qtde_resultados > 0 THEN null
                        WHEN pbh.tratada = false THEN null
                            ELSE pbh.updated_at::time
                                END) as hora_tratada, 
                (CASE WHEN pbh.qtde_resultados > 0 THEN '-'
                        WHEN pbh.tratada = false THEN '-'
                            ELSE ut.nome
                                END) as usuario_tratou,                                
            ...
            LEFT JOIN usuarios      ut ON ut.id = pbh.usuario_alteracao_id 
