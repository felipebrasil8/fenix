    ...
    public function up()
    {
        DB::statement("CREATE OR REPLACE VIEW tickets_view AS
            SELECT 
                ...
                ts.nome AS status_nome, ts.cor AS status_cor, ts.ordem AS status_ordem, ts.nome AS status_aberto, tp.id AS prioridade_id, tp.nome AS prioridade_nome, tp.cor AS prioridade_cor, tp.prioridade AS prioridade_ordem, d.id AS departamento_id, d.nome AS departamento_nome
            FROM tickets AS t
            ...
            INNER JOIN funcionarios AS fs ON fs.id = us.id
            LEFT JOIN usuarios AS ur ON ur.id = t.usuario_responsavel_id
            LEFT JOIN funcionarios AS fr ON fr.id = ur.id
