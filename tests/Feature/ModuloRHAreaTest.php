...
class ModuloRHAreaTest extends ModuloTest
{
    ...
    // Arquivos de view
    private $view = 'rh.area';

    // Variavies de dados
    private $dados = ['nome' => 'TESTE AUTOMATIZADO - CADASTRO', 'descricao' => 'DESCRIÇÃO DO CARGO', 'gestor' => 1 ];
    private $dadosUpdate = ['nome' => 'TESTE AUTOMATIZADO - UPDATE', 'descricao' => 'DESCRIÇÃO DO CARGO', 'gestor' => ['id' => 1 ] ];
    ...
    public function testSearch()
    {
        ...
        $this->assertNotEmpty($response->getData()->areas->data);

        // Login Admin - nenhum resultado
    	$this->search['nome'] = 'XXXXXXXXXXXXXXXXX';
        $response = $this->getUserAdmin()->json('POST',$this->dir.'/search',$this->search);
        $response->assertStatus(200);
        $this->assertEmpty($response->getData()->areas->data);
    ...
    private function getId($nome){
        ...
        return $this->getUserAdmin()->json('POST',$this->dir.'/search',$this->search)->getData()->areas->data[0]->id;
    }
