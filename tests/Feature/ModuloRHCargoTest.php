...
class ModuloRHCargoTest extends ModuloTest
{

    // Diretorio do módulo
    private $dir = '/rh/cargo';

    // Arquivos de view
    private $view = 'rh.cargo';

    // Variavies de dados
    private $dados = ['nome' => 'TESTE AUTOMATIZADO - CADASTRO', 'descricao' => 'DESCRIÇÃO DO CARGO', 'gestor' => 1, 'departamento' => 1 ];
    private $dadosUpdate = ['nome' => 'TESTE AUTOMATIZADO - UPDATE', 'descricao' => 'DESCRIÇÃO DO CARGO', 'gestor' => ['id' => 1 ], 'departamento' => ['id' => 1 ] ];
    ...
    public function testSearch()
    {
        ...
        $this->assertNotEmpty($response->getData()->cargos->data);

        // Login Admin - nenhum resultado
    	$this->search['nome'] = 'XXXXXXXXXXXXXXXXX';
        $response = $this->getUserAdmin()->json('POST',$this->dir.'/search',$this->search);
        $response->assertStatus(200);
        $this->assertEmpty($response->getData()->cargos->data);
    ...
    private function getId($nome){
        ...
        return $this->getUserAdmin()->json('POST',$this->dir.'/search',$this->search)->getData()->cargos->data[0]->id;
    }
